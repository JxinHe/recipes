   ...
   loop_->assertInLoopThread();
    ...
    if(!channel_->isWriting()&&outputBuffer_.readableBytes()==0)
    {
      ...
      if(nwrote>=0)
      {
       ...
       {
        LOG_TRACE<<"I am goingto write more data:'
       }
      ...
      assert(nwrote>=0);
      if(implicit_cast<size_t>(nworte)<message.size()
         {
...
void TcpConnection::setTcpNoDelay(bool on)
         {
          socker_->setTcpNoDelay(on); 
         }

void TcpConnection::connectEstablished()
         {
          loop_->asserttInLoopThread();
           asser(state_==kConnecting);
           setState(kConnected);
           channel_->enableReading();
           connectionCallback_(shared_from_this());
         }

void TcpConnection::connecDestroyed())
      {
       loop_->assertInLoopThread();
        assert(state_==kConnected||state_==kDisconnecting);
        setState(kDisconnnected);
        channel_->disableAll();
        connectionCallback_(shared_from_this());
        loop_->removeChannel(get_pointer(channel_));
      }
      
void TcpConnection::handleRead(Timerstamp receiveTime)
{
 int savedErrno=0;
...
void TcpConnection::handleWrite()
{
 loop_->asserINLOopThread();
  if(channel_->isWriting()
     {
ssize_t n=::write(channel_->fd(),outputBuffer_.peek(),outputBuffer_.readbleBytes());
     }
     if(n>0)
     {
...
loop_queueInLoopt(
  ...
  void TcpConnection::handleClose()
  {
   loop_->assertInLoopThread();
    ...
    assert(state_==kConnected||start_==kDisconneting);
    channel_->disableAll();
    closeCallback_(shared_from_this());
  }
  void TcpConnection::handleError()
  {
   int err=sockers::getSocketError(channel_->fd());
